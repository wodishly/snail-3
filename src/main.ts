...
window.onload = () => {
  ...
  const time = 0;
  const temp = { a: 0, b: 0 };
  const alwaysVoice = true;
  const autoWobble = true;
  const noiseFreq = 500;
  const noiseQ = 0.7;
  const palePink = "#FFEEF5";

  const ui = makeUi(backCanvas, tractCanvas);
  const AudioSystem = {
    blockLength: 512,
    blockTime: 1,
    started: false,
    ...
    doScriptProcessor: function (event) {
      ...
      for (const j = 0, N = outArray.length; j < N; j++) {
        const lambda1 = j / N;
  ...
  const Glottis = {
    ...
    drawKeyboard: function () {
      ...
      for (const i = 0; i < this.semitones; i++) {
    ...
    handleTouches: function () {
      ...
      if (this.touch == 0) {
        for (const j = 0; j < ui.touchesWithMouse.length; j++) {
      ...
      if (this.touch != 0) {
        const local_y = this.touch.y - this.keyboardTop - 10;
        ...
        const t = Math.clamp(1 - local_y / (this.keyboardHeight - 28), 0, 1);
    ...
    runStep: function (lambda, noiseSource) {
      ...
      const out = this.normalizedLFWaveform(
        this.timeInWaveform / this.waveformLength,
      );
      const aspiration =
        this.intensity *
    ...
    setupWaveform: function (lambda) {
      ...
      const RHSIntegral = (1 / epsilon) * (shift - 1) + (1 - Te) * shift;
  ...
  const Tract = {
    ...
    init: function () {
      ...
      for (const i = 0; i < this.noseLength; i++) {
        const diameter;
    ...
    calculateNoseReflections: function () {
      ...
      for (const i = 1; i < this.noseLength; i++) {
    ...
    runStep: function (glottalOutput, turbulenceNoise, lambda) {
      ...
      r =
        this.newReflectionRight * (1 - lambda) + this.reflectionRight * lambda;
    ...
    processTransients: function () {
      for (const i = 0; i < this.transients.length; i++) {
      ...
      for (const i = this.transients.length - 1; i >= 0; i--) {
        const trans = this.transients[i];
    ...
    addTurbulenceNoiseAtIndex: function (turbulenceNoise, index, diameter) {
      ...
      const thinness0 = Math.clamp(8 * (0.7 - diameter), 0, 1);
