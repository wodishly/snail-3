...
export var Glottis = {
  ...
  runStep: function (lambda, noiseSource) {
    var timeStep = 1.0 / AudioSystem.sampleRate;
  ...
  finishBlock: function () {
    ...
    this.newTenseness =
      this.UITenseness +
      0.1 * simplex1(this.totalTime * 0.46) +
      0.05 * simplex1(this.totalTime * 0.36);
    if (!this.isTouched && UI.alwaysVoice)
      this.newTenseness += (3 - this.UITenseness) * (1 - this.intensity);

    if (this.isTouched || UI.alwaysVoice) this.intensity += 0.13;
